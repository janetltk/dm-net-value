...
last <- merge(d[, c("serial_no", "yr")], s[, c("serial_no", "yr", "predicted_year")], all.x = TRUE) # merge spending data with participants' death yr to get spending in last year of life
last <- as.data.table(last)
last <- last[, mean(predicted_year), keyby = yr] # get average spending of last yr of life for each yr
...
nonlast <- merge(d[, c("serial_no", "yr")], s[, c("serial_no", "yr", "predicted_year")], all.x = TRUE, by = "serial_no")
...
eachyr <- nonlast[!(yr.x == "2007") & yr.y == "2007", mean(predicted_year)]

for (i in 2008:2013){
  eachyr <- rbind(eachyr, nonlast[!(yr.x <= i) & yr.y == i, mean(predicted_year)])
}
