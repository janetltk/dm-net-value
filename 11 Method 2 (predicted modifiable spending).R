...
# tidy up predicted spending dataframe
s <- s[, -c("bl.spend.mod", "fn.spend.mod", "diff.spend.mod", "bl.spend.yr", "fn.spend.yr", "diff.spend.yr")]
s <- reshape(s, direction = "long", varying = c("predicted_modifiable.2006", "predicted_year.2006", "predicted_modifiable.2007", "predicted_year.2007", "predicted_modifiable.2008", "predicted_year.2008", "predicted_modifiable.2009", "predicted_year.2009", "predicted_modifiable.2010", "predicted_year.2010", "predicted_modifiable.2011", "predicted_year.2011", "predicted_modifiable.2012", "predicted_year.2012", "predicted_modifiable.2013", "predicted_year.2013", "predicted_modifiable.2014", "predicted_year.2014"))
names(s) <- c("serial_no", "yr", "predicted_modifiable", "predicted_year", "id")
s$yr <- as.factor(s$yr)
...
last <- merge(d[, c("serial_no", "yr")], s[, c("serial_no", "yr", "predicted_modifiable")], all.x = TRUE) # merge spending data with participants' death yr to get spending in last year of life
last <- as.data.table(last)
last <- last[, mean(predicted_modifiable), keyby = yr] # get average spending of last yr of life for each yr
...
nonlast <- merge(d[, c("serial_no", "yr")], s[, c("serial_no", "yr", "predicted_modifiable")], all.x = TRUE, by = "serial_no")
...
eachyr <- nonlast[!(yr.x == "2007") & yr.y == "2007", mean(predicted_modifiable)]

for (i in 2008:2013){
  eachyr <- rbind(eachyr, nonlast[!(yr.x <= i) & yr.y == i, mean(predicted_modifiable)])
}
